...
const createRaceRowHTML = (result, index) => `
    <tr>
        ...
        <!-- <td><a href="${
            result.Constructor.url
        }">Constructor Profile</a></td> -->
    </tr>
`;
...
export default class F1Season {
    ...
    initializeRaces() {
        ...
        this.races.forEach((race, index) => {
            ...
            const raceLink = createLinkElement(
                raceName,
                "race-link",
                async (e) => {
                    e.preventDefault();
    ...
    async populateMainContent(details) {
        ...
        const averagePrecipitation =
            weatherData.hourly.precipitation.reduce(
                (acc, val) => acc + val,
                0
            ) / weatherData.hourly.precipitation.length;
        const roundedTemperature = parseFloat(averageTemperature).toFixed(2);
        const roundedPrecipitation =
            parseFloat(averagePrecipitation).toFixed(2);
        ...
        setTimeout(() => {
            let tableRowsHTML = details.Results.slice(0, 5)
                .map(createRaceRowHTML)
                .join("");
...
`;
    ...
    async initializeSeasonStats() {
        ...
        setTimeout(() => {
            ...
            imageElement.src =
                "https://upload.wikimedia.org/wikipedia/commons/thumb/7/75/Gerhard_Berger_1988_Canada.jpg/220px-Gerhard_Berger_1988_Canada.jpg"; // Replace this
        ...
        `;
            ...
            const canvas = chartContainer.appendChild(
                document.createElement("canvas")
            );

            const podiumsData = podiums1988.map((item) => item.podiums);
            const constructorNames = podiums1988.map(
                (item) => item.constructor
            );
            const backgroundColors = podiums1988.map(
                (item) => constructorColors[item.constructor]
            );
            ...
            const scatterCanvas = scatterPlotContainer.appendChild(
                document.createElement("canvas")
            );
            ...
            const doughnutCanvas = doughnutChartContainer.appendChild(
                document.createElement("canvas")
            );
                    ...
                    datasets: [
                        {
                            ...
                            backgroundColor: Object.keys(
                                driverNationalities
                            ).map((nat) => nationalityColors[nat] || "#FFFFFF"),
                            borderColor: "rgba(0, 0, 0, 0.1)",
                            borderWidth: 1,
                        },
                    ],
